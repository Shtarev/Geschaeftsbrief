    ...
    if (isset($_FILES['filename']) && $_FILES['filename']['name'] != ''){
        ...
        if($ext == '.ttf' || $ext == '.otf'){
            ...
            if($result == true) {
                unlink($name.'.z');
                unlink($file);
                if($enc == 'cp1252'){$de='В utf-8 файлах, для отображения умляутов в немецкоязычных текстах, перед выводом текста пропускайте его через одну из функций: <pre>$txt = iconv(\'utf-8\', \'cp1252\', $txt);</pre><pre>$txt = mb_convert_encoding($txt, \'cp1252\', \'utf-8\');</pre><pre>$txt = utf8_decode($txt);</pre>Очень удобно, если один из этих метоов прописать именно в вышеуказанном виде в файле fpdf.php над линией кода if($align==\'R\'), тогда эту функцию не надо будет прописывать в создаваемом файле, она будет срабатывать сама автоматически каждый раз при выводе текста.<br><br>В методе <code>AddFont()</code> отсутствует возможность установки жирного текста и курсивного текста, например такая попытка установить жирный шрифт: <code>$pdf->AddFont(\''.ucfirst($name).'\',\'B\',\''.$name.'.php\');</code> вызовет ошибку.<br>Для использования возможностей выделять текст жрным или курсивом установите отдельные файлы жирного и курсивного текста и подключайте их в нужных местах. Подчеркивание установки дополнительных файлов не требует.';}
                if($enc == 'cp1251'){$de='В utf-8 файлах, для отображения кириллических текстов, а не кракозябр, перед выводом текста пропускайте его через одну из функций: <pre>$txt = iconv(\'utf-8\', \'cp1251\', $txt);</pre><pre>$txt = mb_convert_encoding($txt, \'cp1251\', \'utf-8\');</pre>Очень удобно, один из этих метоов прописать именно в вышеуказанном виде в файле fpdf.php над линией кода if($align==\'R\'), тогда эту функцию не надо будет прописывать в создаваемом файле, она будет срабатывать сама автоматически каждый раз при выводе текста.<br><br>В методе <code>AddFont()</code> отсутствует возможность установки жирного текста и курсивного текста, например такая попытка установить жирный шрифт: <code>$pdf->AddFont(\''.ucfirst($name).'\',\'B\',\''.$name.'.php\');</code> вызовет ошибку.<br>Для использования возможностей выделять текст жрным или курсивом установите отдельные файлы жирного и курсивного текста и подключайте их в нужных местах. Подчеркивание установки дополнительных файлов не требует.';}
